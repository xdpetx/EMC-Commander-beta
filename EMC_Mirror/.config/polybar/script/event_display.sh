...
# This is the event handler for [module/display]

#notify-send "display_event_handler:script started"
...
case $BUTTON in
	# edit this begin
	left	 ) "$_SCRIPTDIR/set_resolution.sh";;
	dbl_left ) eval $(exec_key display_dbl_left) &;;
	right	 ) eval $(exec_key display_right) &;;
	dbl_right) eval $(exec_key display_dbl_right) &;;
	up		 ) show_display_click_actions ;;
...
show_display_click_actions() {

	local connection=$(LC_ALL=C xrandr | grep -w connected)
	connection="${connection%% *}"

	# colors from polybar config.ini
	local color_primary="#F0C674" color_bg="#282A2E" color_fg="#C5C8C6"
	local click_action="" display_icon="󰍺"
	local display_icon_colored="<span background='$color_bg' foreground='$color_primary'>$display_icon</span>"
	local display_output="<span background='$color_bg' foreground='$color_fg'> $connection </span>"

	click_action+="\<b>$display_icon_colored$display_output Modul DISPLAY</b> Help

    click left  $display_icon_colored: set resolution
    click right $display_icon_colored: $(get_module_clicks display_right)
dbl click left  $display_icon_colored: $(get_module_clicks display_dbl_left)
dbl click right $display_icon_colored: $(get_module_clicks display_dbl_right)

    click left/right on $display_output show/hide Info
dbl click left       on $display_output open Info box
"
