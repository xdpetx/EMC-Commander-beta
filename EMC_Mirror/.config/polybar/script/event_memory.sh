...
# This is the event handler for [module/memory]

#notify-send "memory_event_handler:script started"
...
case $BUTTON in
	# edit this begin
	left	 ) eval $(exec_key memory_left) & ;;
	dbl_left ) eval $(exec_key memory_dbl_left) & ;;
	right	 ) eval $(exec_key memory_right) & ;;
	dbl_right) eval $(exec_key memory_dbl_right) & ;;
	up		 ) show_memory_click_actions ;;
...
show_memory_click_actions() {

	local color_primary="#F0C674" color_bg="#282A2E" color_fg="#C5C8C6"
	local click_action="" memory_icon="󰘚"
	local memory_icon_colored="<span background='$color_bg' foreground='$color_primary'>$memory_icon</span>"
	local memory_output="<span background='$color_bg' foreground='$color_fg'> 28% </span>"

	click_action+="\<b>$memory_icon_colored$memory_output Modul MEMORY</b> Help

    click left  $memory_icon_colored: $(get_module_clicks memory_left)
    click right $memory_icon_colored: $(get_module_clicks memory_right)
dbl click left  $memory_icon_colored: $(get_module_clicks memory_dbl_left)
dbl click right $memory_icon_colored: $(get_module_clicks memory_dbl_right)

    click left/right on $memory_output show/hide Info
dbl click left       on $memory_output open Info box
"
