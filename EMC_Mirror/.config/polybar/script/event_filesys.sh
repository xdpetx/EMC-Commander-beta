...
# This is the event handler for [module/filesys]

#notify-send "filesys_event_handler:script started"
...
case $BUTTON in
	# edit this begin
	left	 ) eval $(exec_key filesys_left) & ;;
	dbl_left ) eval $(exec_key filesys_dbl_left) & ;;
	right	 ) eval $(exec_key filesys_right) & ;;
	dbl_right) eval $(exec_key filesys_dbl_right) & ;;
	up		 ) show_filesys_click_actions ;;
...
show_filesys_click_actions() {

	local color_primary="#F0C674" color_bg="#282A2E" color_fg="#C5C8C6"
	local click_action="" filesys_icon="󰋊" 
	local filesys_icon_colored="<span background='$color_bg' foreground='$color_primary'>$filesys_icon</span>"
	local filesys_output="<span background='$color_bg' foreground='$color_fg'> /10% </span>"

	click_action+="\<b>$filesys_icon_colored$filesys_output Modul FILESYS</b> Help

    click left  $filesys_icon_colored: $(get_module_clicks filesys_left)
    click right $filesys_icon_colored: $(get_module_clicks filesys_right)
dbl click left  $filesys_icon_colored: $(get_module_clicks filesys_dbl_left)
dbl click right $filesys_icon_colored: $(get_module_clicks filesys_dbl_right)

    click left/right on $filesys_output show/hide Info
dbl click left       on $filesys_output open Info box
"
