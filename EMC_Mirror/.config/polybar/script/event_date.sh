...
# This is the event handler for [module/date]

#notify-send "date_event_handler:script started"
...
case $BUTTON in
	# edit this begin
	left	 ) eval $(exec_key date_left) & ;;
	dbl_left ) eval $(exec_key date_dbl_left) & ;;
	right	 ) eval $(exec_key date_right) & ;;
	dbl_right) eval $(exec_key date_dbl_right) & ;;
	up		 ) show_date_click_actions ;;
...
show_date_click_actions() {

	local time=$(date +%R)
	# colors from polybar config.ini
	local color_primary="#F0C674" color_bg="#282A2E" color_fg="#C5C8C6"
	local click_action="" date_icon=""
	local date_icon_colored="<span background='$color_bg' foreground='$color_primary'>$date_icon</span>"
	local date_output="<span background='$color_bg' foreground='$color_fg'> $time </span>"



	click_action+="\<b>$date_icon_colored$date_output Modul DATE</b> Help

    click left  $date_icon_colored: $(get_module_clicks date_left)
    click right $date_icon_colored: $(get_module_clicks date_right)
dbl click left  $date_icon_colored: $(get_module_clicks date_dbl_left)
dbl click right $date_icon_colored: $(get_module_clicks date_dbl_right)

    click left/right on $date_output show/hide Info
"
